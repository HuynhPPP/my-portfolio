  ...
  return (
    ...
    >
      <nav className="max-w-6xl mx-auto flex items-center justify-between px-4 py-3">
        <a href="#home" className="font-bold text-base md:text-lg">
        ...
        <ul className="hidden sm:flex gap-5 md:gap-6 text-sm md:text-[0.95rem]">
          {navItems.map((item) => (
            <li key={item.href}>
      ...
      {open && (
        <ul className="sm:hidden px-4 pb-3 space-y-2 text-sm">
          {navItems.map((item) => (
            <li key={item.href}>
